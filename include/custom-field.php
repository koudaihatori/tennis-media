...
function insert_jin_seotitle(){
	...
	wp_nonce_field(wp_create_nonce(__FILE__), 'my_nonce');
	echo '<table style="text-align:left;"><tbody>';
	echo '<tr><td><span style="font-size:0.8em; color:#888;">記事タイトルとは別に、この記事のtitleタグを設定することができます。</span></td></tr><tr><td><input name="jin_seotitle" size="50%" value="'.esc_html(get_post_meta($post->ID, 'jin_seotitle', true)).'" /></td></tr>';
	echo '</tbody></table>';
}
...
function insert_mydesc(){
	...
	wp_nonce_field(wp_create_nonce(__FILE__), 'my_nonce');
	echo '<table style="text-align:left;"><tbody>';
	echo '<tr><td><span style="font-size:0.8em; color:#888;">この記事がどういった内容なのか、検索エンジンやユーザーに向けて説明を書くことができます。</span></td></tr><tr><td><textarea name="post_desc" rows="5" cols="60" >'.esc_html(get_post_meta($post->ID, 'post_desc', true)).'</textarea></td></tr>';
	echo '</tbody></table>';
}
...
function insert_jin_keyword(){
	...
	wp_nonce_field(wp_create_nonce(__FILE__), 'my_nonce');
	echo '<table style="text-align:left;"><tbody>';
	echo '<tr><td><span style="font-size:0.8em; color:#888;">この記事のメタキーワードを設定できます。</span></td></tr><tr><td><input name="jin_keyword" size="50%" value="'.esc_html(get_post_meta($post->ID, 'jin_keyword', true)).'" /></td></tr>';
	echo '</tbody></table>';
}
